...
function fixCustomPlugins(doc, rewardsLaunchURL) {
  ...
  doc.previewContainer.contents().each(function () {
    ...
    } else if (ctrlcRegex.test(content)) {
      const { valid } = parseDirective(content, []);
      if (valid) {
        const text = `Send a CTRL+C to Terminal`;
        tag.context.innerHTML = `<button class="plugin" onclick="ctrlC()"><i class="fa fa-times-circle"></i> <span class="plugin-text">${text}</span></button>`;
      } else {
        tag.remove();
      }
    }
